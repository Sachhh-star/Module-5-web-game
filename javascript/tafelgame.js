...
function backToMenu() {
  ...
  stopTimer(); // Stop sound when returning to menu
  switchScreen("start");
}
...
function startTimer() {
  ...
  stopTimer(); // Stop any lingering sound
  ...
  timerInterval = setInterval(() => {
    ...
    if (timer <= 0) {
      clearInterval(timerInterval);
      stopTimer(); // Stop sound because time is up
      handleTimeOut();
    }
...
function generateAnswers(num1, num2) {
  ...
  while (answers.length < 4) {
    ...
    if (strategy === 1) {
      ...
      let fakeNum =
        (Math.random() > 0.5 ? num1 : num2) + (Math.random() > 0.5 ? 1 : -1);
...
function checkAnswer(selectedAnswer, btnElement) {
  ...
  isAnimating = true;
  clearInterval(timerInterval);
  stopTimer(); // Stop the sound when an answer is selected
