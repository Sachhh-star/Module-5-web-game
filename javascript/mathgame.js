...
function checkAnswer() {
  ...
  isChecking = true;
  clearInterval(timerInterval);
  stopTimer(); // Cut sound off immediately when checking
  ...
  if (userAnswer === currentCorrectAnswer) {
    ...
    feedbackMsg.innerHTML =
      "Wrong! The right answer was " + currentCorrectAnswer;
    feedbackMsg.style.color = "red";
    let lowScore = level * 10;
  ...
  if (score <= 0) {
    score = 0;
    feedbackMsg.innerHTML += " Game Over! Your score reset to 0. Try again!";
    feedbackMsg.style.color = "red";
...
function countdown() {
  if (timeSecounds > 0) {
    ...
    clearInterval(timerInterval);
    stopTimer(); // Stop sound on timeout
    isChecking = true;
